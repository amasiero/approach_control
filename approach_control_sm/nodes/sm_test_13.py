...
def setup_sm():
    ...
    with sm:
        ...
        smach.StateMachine.add('SAD_FACE', PublishFace.PublishFace('angry_red'),
        ...
        with sm_con_1:
          ...
          smach.Concurrence.add('DAMMED', Say.Say("DAMMED The bootle is not here too"))

        smach.StateMachine.add('CON_1', sm_con_1,
                                transitions={'success':'SOFA_F', 'fail':'CON_1'})

        smach.StateMachine.add('SOFA_F', GoToLocation.GoToLocation('sofa_perto'),
                               transitions={'success':'SAD_FACE_2','fail':'Done'})

        smach.StateMachine.add('SAD_FACE_2', PublishFace.PublishFace('angry_red'),
        ...
        with sm_con_2:
          ...
          smach.Concurrence.add('HI', Say.Say("Human"))

        smach.StateMachine.add('CON_2', sm_con_2,
                                transitions={'success':'QUESTION', 'fail':'CON_2'})

        smach.StateMachine.add('QUESTION', Say.Say("Find my bottle"),
                               transitions={'spoke' : 'FOLLOW', 'mute' : 'Done'})

        smach.StateMachine.add('FOLLOW', Say.Say("Follow me outside now"),
                               transitions={'spoke' : 'EXIT', 'mute' : 'Done'})

        smach.StateMachine.add('EXIT', GoToLocation.GoToLocation('saida'),
                               transitions={'success':'FINISHED','fail':'Done'})

        smach.StateMachine.add('FINISHED', Say.Say("You finished the test"),
